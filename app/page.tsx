...
export default function DashboardPage() {
  ...
  const typeIcons: Record<string, string> = { page: "\u{1F4C4}", video: "\u{1F3A5}", tweet: "\u{1F4AC}", pdf: "\u{1F4D1}", screenshot: "\u{1F4F7}", transcript: "\u{1F399}" };
    ...
    <>
      ...
      <div className="main-container">
        ...
        <div className="ai-ask-bar">
          <div className="ai-ask-icon">{"\u{1F9E0}"}</div>
          <input
            type="text"
        ...
        <div className="stats-bar-row">
          ...
          <div className="big-stats">
            <div className="big-stat">
              ...
              <div className="big-stat-label"><span className="big-stat-icon">{"\u{1F4C4}"}</span>Total</div>
            </div>
            <div className="big-stat">
            ...
            <div className="big-stat">
              ...
              <div className="big-stat-label"><span className="big-stat-icon">{"\u{1F3F7}"}</span>Tags</div>
            </div>
          </div>
        </div>
        ...
        <div className="dashboard-grid">
          ...
          <div className="card timer-card">
            ...
            <div className="timer-controls">
              <button className="timer-btn" title="Pages">{"\u{1F4C4}"}</button>
              <button className="timer-btn primary" title="All Items">{"\u{1F4DA}"}</button>
              <button className="timer-btn" title="Videos">{"\u{1F3A5}"}</button>
            </div>
          </div>
              ...
              {recentItems.length === 0 ? (
                ...
                recentItems.map((item) => (
                  <div className="task-item" key={item._id}>
                    <div className="task-icon">{typeIcons[item.type] || "\u{1F4C4}"}</div>
                    <div className="task-info">
                ...
                <div className={`item-card ${item.duplicateOf ? "item-card-dupe" : ""}`} key={item._id}>
                  ...
                  <div className="item-footer">
                    ...
                    <div className="item-actions">
                      <button className="btn-icon" onClick={() => router.push(`/items/${item._id}`)} title="View Details">{"\u{1F441}"}</button>
                      {!item.summary && (
                        <button className="btn-icon" onClick={() => summarizeItem(item._id)} title="AI Summarize">{"\u{1F9E0}"}</button>
                      )}
                      <button className="btn-icon" onClick={() => loadRelated(item._id)} title="Related Items">{"\u{1F517}"}</button>
                      ...
                      <button className="btn-icon" onClick={() => setReminderItemId(item._id)} title="Set Reminder">{"\u{1F514}"}</button>
                      ...
                      <button className="btn-icon" onClick={() => setDeleteId(item._id)} title="Delete">{"\u{1F5D1}"}</button>
                    </div>
                  </div>
                  {/* Related Items Panel */}
